package kr.co.express9.client.thirdParty.kakao

import android.content.Context
import com.kakao.auth.*


class KakaoSDKAdapter(private val applicationContext: Context) : KakaoAdapter() {

    override fun getApplicationConfig(): IApplicationConfig {
        return IApplicationConfig { applicationContext }
    }

    override fun getSessionConfig(): ISessionConfig {
        return object : ISessionConfig {

            /**
             * 로그인 웹뷰에서 email 입력 폼의 데이터를 저장할 지 여부
             */
            override fun isSaveFormData(): Boolean {
                return true
            }

            /**
             * 로그인 인증 타입
             * KAKAO_TALK  : 카카오톡 로그인 타입
             * KAKAO_STORY : 카카오스토리 로그인 타입
             * KAKAO_ACCOUNT : 웹뷰 다이얼로그를 통한 계정연결 타입
             * KAKAO_TALK_EXCLUDE_NATIVE_LOGIN : 카카오톡 로그인 타입과 함께 계정생성을 위한 버튼을 함께 제공
             * KAKAO_LOGIN_ALL : 모든 로그인 방식을 제공
             */
            override fun getAuthTypes(): Array<AuthType> {
                return arrayOf(AuthType.KAKAO_TALK)
            }

            /**
             * 로그인 웹뷰에서 pause와 resume시에 타이머를 설정하여, CPU의 소모를 절약여부
             * true로 지정할 경우, 로그인 웹뷰의 onPuase()와 onResume()에 타이머 설정 필
             */
            override fun isUsingWebviewTimer(): Boolean {
                return false
            }

            /**
             * 로그인 시 토큰을 저장할 때의 암호화 여부
             */
            override fun isSecureMode(): Boolean {
                return false
            }

            /**
             * 일반 사용자가 아닌 Kakao와 제휴 된 앱에서 사용되는 값
             * 값을 지정하지 않을 경우, ApprovalType.INDIVIDUAL 값으로 사용
             */
            override fun getApprovalType(): ApprovalType? {
                return ApprovalType.INDIVIDUAL
            }
        }
    }
}